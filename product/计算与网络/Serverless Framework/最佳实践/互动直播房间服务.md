...
[秀场直播/电商直播](https://cloud.tencent.com/document/product/454/38625) 通过云函数模板LiveRoom创建业务后台、配合移动直播SDK、即时通信IM 低门槛快速实现登录、注册、开播、连麦一整套在线直播业务场景。
 ...
 **私有网络**：vpc名称。
  ...
  **SDKappid**：TRTC的应用名称，可以在trtc控制台获取。
 
**密钥**：TRTC的密钥，可以在跑通demo界面获取到。

**数据库地址**：用来存储用户信息，这里一般使用mysql，和mysql的地址保持一致即可。

**数据库名称**：指数据库应用名，保持一致即可。

**redis地址**：redis数据库的访问地址，该数据用来保存用户状态。
 
 4、参数全部选好，点击完成，创建房间服务。
     
######   方案二 命令行创建（默认创建TD SQL）
...
说明：默认api接口是免鉴权的，如果对于安全性有较高的要求，可以在网关控制台更改鉴权模式，具体参考[选择鉴权方式](https://cloud.tencent.com/document/product/628/64198)，建议选择应用鉴权，鉴权算法参考[应用鉴权](https://cloud.tencent.com/document/product/628/55088)
